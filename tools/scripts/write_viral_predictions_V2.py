...
def virus_pred(assembly_file, output_dir, virome_dataset, prok_mode):
        ...
        if virome_dataset:
                ...
                subprocess.call("wrapper_phage_contigs_sorter_iPlant.pl -f %s --db 2 --wdir %s --virome --data-dir %s" % (assembly_file, output_dir, virsorter_data_path), stdout = subprocess.DEVNULL, stderr = subprocess.DEVNULL, shell = True)
        else:
                print("Running VirSorter without virome decontamination mode")
                subprocess.call("wrapper_phage_contigs_sorter_iPlant.pl -f %s --db 2 --wdir %s --data-dir %s" % (assembly_file, output_dir, virsorter_data_path), stdout = subprocess.DEVNULL, stderr = subprocess.DEVNULL, shell = True)
	HC_viral_predictions = []
        ...
        VirSorter_viral_files = [x for x in glob.glob(os.path.join(virsorter_dir, "Predicted_viral_sequences", "*")) if re.search(r"cat-[12]\.fasta", x)]
